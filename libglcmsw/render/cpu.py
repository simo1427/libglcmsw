...
func glcmpropnew:
  ...
  Arguments:
    glcm - a normalized GLCM, GENERATED BY ONEGLCM
    prop - a string containing the desired property

  Process:
    check for each property and calculate it
...
"""
func oneglcm:
  returns a float64 array - calculated GLCM of a given image

  Arguments:
    img - input image
    dist - distance between the reference and neighbour pixel
    angle - angle between the x-axis and the distance line
    bitdepth - number of gray levels in the input image
    kwargs:
      normed=False - whether the matrix whould be normalized or not
      symmetric=False - whether the matrix should be symmetrix along the main diagonal

  Process:
    create an empty square array with the size of the bitdepth
    calculate the neighbourhood in distance along the x-axis and distance along the y-axis
    series of if statements thet determine the start and end position of array iteration
    iterate through the array and generate the GLCM
    make it symmetric
    normialize
...
func singletilecpunew:
  ...
  iterate through the input image:
    ...
    calculate property and store the value in the respective column
    store the temporary array in the respective row of the array with the results

  Usage:
    libglcmsw.render.cpu.tilerenderlist
"""
@njit(nogil=True)
def singletilecpu(im, windowsz, prop,angle, dist,bitdepth):
  ...
  for ii in range(ri):
    ...
    for jj in range(rj):
      img = im[ii:ii + windowsz, jj:jj + windowsz]#extract part of the image
...
func singleline:
  ...
  windowsz - size of sliding window
  angle - angle of GLCM (0,45,90,...)
  dist - distance of GLCM
...
"""
@njit(nogil=True)
def singleline(ii,rj,im,prop,windowsz,angle, dist, bitdepth): # processingi of 1 row of the image
  tmp = np.empty((rj), dtype=np.float32)
  tmp = np.empty((rj,), dtype=np.float32)
  for jj in range(rj):
    img = im[ii:ii + windowsz, jj:jj + windowsz]  # extract part of the image
...
def rasterrender(osobj,windowsz,**kwargs):
  ...
  with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
    ...
    print(inp)
    results = executor.map(singleline, inp,itertools.repeat(rj), itertools.repeat(im), itertools.repeat(prop), itertools.repeat(windowsz),itertools.repeat(angle), itertools.repeat(distance), itertools.repeat(bitdepth))
