...
"""
class SlidingWindow:
    def __init__(self,fname,windowsz,prop, downscale=1):
    ...
    def render(self,mode,tmpdir="./slidingwindowtmp",**kwargs):
        ...
        if not (mode == "raster" or mode == "tiling"):
            ...
            self.result = render.cpu.rasterrender(self.img, self.windowsz, ncores=workers, prop=self.prop, rowssave=rowssave)
        else:
            ...
            if listoftiles:
                render.cpu.tilerenderlist(tmpdir, listoftiles, self.windowsz, ncores=workers, prop=self.prop)
    ...
    def savetif(self, fname, type="float32"):
        ...
        if type=="float32":
            ...
            raise SlidingWindowError("Unsupported data type. Manually save with any function from image libraries (e.g. scikit-image)")