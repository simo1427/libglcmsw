...
def convolve(image, kernels, context, prgs):
    ...
    for ibatch in range(image.shape[0]//batch+1):
        ...
        print((img.shape[1], img.shape[2],img.shape[0]))
        completedEvent=prgs.convolvecut(queue, ((img.shape[1]//patchsz+1)*patchsz, (img.shape[2]//patchsz+1)*patchsz,img.shape[0]), (patchsz, patchsz,1), *krn_args)
