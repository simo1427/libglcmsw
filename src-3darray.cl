...
__kernel void convolvecut(__global uchar *img, __global float *res, __constant float *krn, int sz0, int sz1, int krnsz, int numkrn)
{
    ...
    for(int k=0;k<numkrn;k++)
    {
        ...
        for(int i=-hfs;i<hfs; i++)
        {
            for(int j=-hfs;j<hfs; j++)
            {
                ...
                sum+=img[addr0*sz1+addr1]*krn[k*krnsz*krnsz+i*krnsz+j];
            }
        }
